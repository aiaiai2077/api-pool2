...
对不支持视觉的目标端点触发图片解析时，修复了以下两点：

- 解析请求会带上图片旁边的用户文字指令：视觉模型能看到“帮我读代码 / 描述截图”这类要求，不再只收到一张裸图。
- 图片逐张解析、逐张注入：每张图独立调用视觉模型，并按 `[图片1 解析内容]`、`[图片2 解析内容]` 序号写回原消息，目标模型可以区分多张图片，不再共用同一段描述。
- 界面截图输出结构化 UI 信息：如果图片是软件界面、网页、控制台或手机截图，视觉模型会额外输出视口尺寸（像素）以及每个可见元素的类型、完整文字/OCR、中心坐标与宽高，供文本模型在 computer use 这类任务里做空间定位。
- 兼容 ChatGPT/Codex 的图片格式：图片可能是 `input_image` 类型，也可能以 `data:image/...;base64,...` 内嵌在字符串（如 `[TOOL]` 转写文本）里，现在都会触发视觉解析；同一张图在同一请求中只解析一次，避免重复消耗。
- 只处理当前轮的图片：只扫描最后一个用户消息及其后的内容，历史轮次里的旧图不会再每次重复触发视觉解析。
- 移除流式占位提示：`[API Pool: 检测到图片...]` / `[图片解析完成...]` 不再作为模型内容发给客户端，避免污染对话输出。

对应测试：`python tests\test_vision_translation.py`。
...
| 套件 | 结果 |
| --- | --- |
| `test_responses_api.py` | 9 通过，0 失败 |
| `test_vision_translation.py` | 4 通过，0 失败 |
